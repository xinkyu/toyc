  ...
  | Binop (op, e1, e2) ->
      ...
      | Imm a, Imm b ->
          let folded = match op with
            ...
            | Div -> Imm (a / b) | Mod -> Imm (a mod b)
            | Eq -> Imm (if a = b then 1 else 0) | Neq -> Imm (if a <> b then 1 else 0)
            | Less -> Imm (if a < b then 1 else 0) | Leq -> Imm (if a <= b then 1 else 0)
            | Greater -> Imm (if a > b then 1 else 0) | Geq -> Imm (if a >= b then 1 else 0)
            | Lor | Land -> failwith "Never touched"
          in (folded, c1 @ c2)
      | _ -> let dst = ftemp () in (dst, c1 @ c2 @ [ Binop (string_binop op, dst, lhs, rhs) ]))
  | Call (f, args) ->
      let codes, oprs =
        List.fold_left (fun (acc_code, acc_opr) arg ->
...
let rec stmt_res (ctx : context) (s : stmt) : stmt_res =
  ...
  | If (cond, tstmt, Some fstmt) ->
      ...
      let lthen, lelse, lend = flabel (), flabel (), flabel () in
      let then_res, else_res = stmt_res ctx tstmt, stmt_res ctx fstmt in
      let raw_then, raw_else = flatten then_res, flatten else_res in
      ...
      (match (then_res, else_res) with Returned _, Returned _ -> Returned code | _ -> Normal code)
  | If (cond, tstmt, None) ->
      let cnd, cc = expr_ir ctx cond in
      let lthen, lskip = flabel (), flabel () in
      let then_code = flatten (stmt_res ctx tstmt) in
      Normal (cc @ [ IfGoto (cnd, lthen); Goto lskip ] @ [ Label lthen ] @ then_code @ [ Label lskip ])
  | While (cond, body) ->
      let lcond, lbody, lend = flabel (), flabel (), flabel () in
...
let func_ir (f : func_def) : ir_func =
  let body_with_return =
    match List.rev f.body with
    | Return _ :: _ -> f.body
    | _ -> f.body @ [Return None] (* FIX: Ensure function always returns *)
  in
  let des_body = match dstmt (Block body_with_return) with Block ss -> ss | _ -> body_with_return in
...
let pblocks (insts : ir_inst list) : ir_block list =
  let rec split acc curr label labelmap insts =
    match insts with
    | [] -> if curr = [] then List.rev acc else failwith "Basic block must end with a terminator"
    | Label l :: rest ->
        let next_label, labelmap' = fIRlabel labelmap l in
        if curr = [] then split acc [Label l] next_label labelmap' rest
        else
          let blk = { label; insts = List.rev curr; terminator = TermSeq next_label; preds = []; succs = [] } in
          split (blk :: acc) [Label l] next_label labelmap' rest
    | Goto l :: rest ->
        let goto_label, labelmap' = fIRlabel labelmap l in
        let blk = { label; insts = List.rev curr; terminator = TermGoto goto_label; preds = []; succs = [] } in
        split (blk :: acc) [] "" labelmap' rest
    | IfGoto (cond, l) :: rest ->
        let then_label, labelmap' = fIRlabel labelmap l in
        let else_label, labelmap'' = fIRlabel labelmap' ("__else" ^ string_of_int !irlabid) in
        let blk = { label; insts = List.rev curr; terminator = TermIf (cond, then_label, else_label); preds = []; succs = [] } in
        split (blk :: acc) [] else_label labelmap'' rest
    | Ret op :: rest ->
        let blk = { label; insts = List.rev curr; terminator = TermRet op; preds = []; succs = [] } in
        split (blk :: acc) [] "" labelmap rest
...
let func_iro (f : func_def) : ir_func_o =
  let body_with_return =
    match f.body with
    | [] -> [Return None]
    | stmts -> (match List.rev stmts with Return _ :: _ -> stmts | _ -> stmts @ [Return None]) (* FIX: Ensure function always returns *)
  in
  ...
  let bodycode = stmt_res ctx0 (Block body_with_return) |> flatten in
